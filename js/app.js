...
(() => {
  ...
  const STORAGE_KEY = 'chaerok_state_v2';
  ...
  const SKIN_MAP = {
    dark: { bg: 'transparent', text: '#ECE7E0', card: '#221F2E' },
    light: { bg: '#ffffff', text: '#1c1a22', card: '#f3f1ec' },
    sepia: { bg: '#f2e8d5', text: '#3a2f1f', card: '#e9dcc0' },
    mint: { bg: '#e3f1ec', text: '#1e332c', card: '#d3e9e0' }
  };
  ...
  const state = {
    ...
    imageMap: new Map(),
    display: { skin: 'dark', fontChar: 'pretendard', fontNarration: 'notoserif', showAvatar: true, bubble: false, stacked: false, fontSize: 15, letterSpacing: 0, lineHeight: 1.75, paraSpacing: 14 }
  };
  ...
  function restorePersisted() {
    try {
      ...
      if (data.messages && data.messages.length) {
        ...
        syncCoverInputs(); syncThemeInputs(); syncDisplayInputs(); applyDisplay();
        renderAll();
  ...
  function runCleaning({ silent = false, snapshot = true } = {}) {
    ...
    if (!silent) { toast(report.length ? '정리를 완료했어요.' : '이미 깔끔한 로그예요.'); renderAll(); }
  }
  ...
  function applyDisplay() {
    ...
    const skin = SKIN_MAP[d.skin] || SKIN_MAP.dark;
    ...
    logFeed.style.setProperty('--log-font-char', FONT_MAP[d.fontChar] || FONT_MAP.pretendard);
    logFeed.style.setProperty('--log-font-narr', FONT_MAP[d.fontNarration] || FONT_MAP.notoserif);
    ...
    logFeed.dataset.stacked = String(d.stacked);
  ...
  function syncDisplayInputs() {
    ...
    optShowAvatar.checked = d.showAvatar; optBubble.checked = d.bubble; optStacked.checked = d.stacked;
  ...
  optStacked.addEventListener('change', () => { state.display.stacked = optStacked.checked; applyDisplay(); persist(); });
  ...
  resetDisplayBtn.addEventListener('click', () => {
    Object.assign(state.display, { skin: 'dark', fontChar: 'pretendard', fontNarration: 'notoserif', showAvatar: true, bubble: false, stacked: false, fontSize: 15, letterSpacing: 0, lineHeight: 1.75, paraSpacing: 14 });
  ...
  function syncThemeInputs() {
    themeBg.value = state.exportTheme.bg; themeBubble.value = state.exportTheme.bubbleBg;
    themeText.value = state.exportTheme.text; themeUseColor.checked = state.exportTheme.useCharacterColor;
  }
  ...
  jsonFileInput.addEventListener('change', async () => {
    ...
    try {
      ...
      if (data.exportTheme) Object.assign(state.exportTheme, data.exportTheme);
      if (data.characterRoles) state.characterRoles = new Map(data.characterRoles);
      syncCoverInputs(); syncThemeInputs();
