...
(() => {
  ...
  const state = {
    ...
    display: {
      ...
      showAvatar: true, bubble: false, stacked: false, useCharacterColor: true,
  ...
  function handleParse(showEmptyWarning = true) {
    ...
    const report = runCleaning({ silent: true, snapshot: false });
    if (report.length) notes.push(...report);
    toast(notes.join(' · '));
  ...
  function syncDisplayInputs() {
    ...
    optBubble.checked = d.bubble; optStacked.checked = d.stacked;
