...
const Exporter = (() => {
  ...
  function buildMessagesHtml(messages, display, characterRoles) {
    ...
    return messages.map(m => {
      ...
      const nameColor = (display.useCharacterColor && m.speaker) ? Utils.colorForName(m.speaker) : 'inherit';
  ...
  <div class="exp-center-body">${bodyHtml}</div>
      ...
      const avatar = !avatarShown ? '' : (m.avatarUrl
  ...
  <div class="exp-row-main">
    <span class="exp-speaker" style="color:${nameColor}">${Utils.escapeHtml(m.speaker || '')}</span>${whisperTag}
...
${state.display.bubble ? `.chaerok-log .exp-row{background:${skin.card};border-radius:10px;padding:10px 14px;}` : ''}
...
${body}
  ...
  function openPrintView(state) {
    ...
    const dynamicStyle = `
      ...
      ${state.display.bubble ? `.exp-row{background:${skin.card};border-radius:10px;padding:10px 14px;}` : ''}
