...
const Parser = (() => {
  ...
  function parseRoll20Html(raw) {
    ...
    leafCandidates.forEach(el => {
      ...
      // 나레이션(desc)인데 원본에 글자색이 직접 지정돼 있으면(예: GM 스크립트가
      // 주황색으로 강조한 안내문) "강조 나레이션"으로 자동 인식한다. 원본 색은
      // 지워서 나중에 "읽기 설정 > 강조 나레이션 색상" 하나로 일괄 제어되게 한다
      // (자동으로 안 잡히거나 잘못 잡힌 경우엔 로그에서 태그를 클릭해 직접 켜고 끌 수 있음).
      if (parsed.type === 'desc' && hasInlineTextColor(parsed.html)) {
        parsed.emphasis = true;
        parsed.html = stripInlineColor(parsed.html);
      }
  ...
  // 화자 없는(desc) 메시지의 html에 인라인 글자색(color)이 직접 지정돼
  // 있는지 확인한다. background-color 는 이미 stripBackgroundColor 에서
  // 지워진 뒤이므로, "color:" 앞에 "background-" 접두어가 없는 경우만 잡는다.
  function hasInlineTextColor(html) {
    if (!html) return false;
    return /(^|;|"|\s)color\s*:\s*[^;"]+/i.test(html);
  }

  // style 속성에서 color(글자색) 규칙만 제거한다 (background-color 등 다른
  // 속성은 그대로 둔다).
  function stripInlineColor(html) {
    return html.replace(/style="([^"]*)"/gi, (full, styleContent) => {
      const kept = styleContent.split(';').map(s => s.trim()).filter(Boolean).filter(rule => {
