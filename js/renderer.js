...
const Renderer = (() => {
  ...
  function renderMessage(m, state) {
    ...
    const avatarColor = m.speaker ? Utils.colorForName(m.speaker) : '#5C5868';
    const useColor = state.display.useCharacterColor !== false;
    const nameColor = (useColor && m.speaker) ? Utils.colorForName(m.speaker) : 'inherit';
    ...
    return `<article class="msg-row msg-row--dialog${emoteClass}${whisperClass}" data-id="${m.id}">
      ${dragHandle}
      ${avatarHtml(m, avatarColor)}
      <div class="dialog-content">
        <span class="dialog-name" style="color:${nameColor}">${Utils.escapeHtml(m.speaker || '')}</span>
        ${whisperTag}
