...
const Renderer = (() => {
  ...
  function renderGroup(u, state) {
    ...
    // 이름은 첫 번째 줄에만 실제로 보여주고, 이어지는 줄들은 같은 이름을
    // 투명(visibility:hidden)하게 깔아 너비를 맞춰서 대사가 이름 옆에서
    // 시작되고, 두 줄로 넘어가도 그 시작 위치에 맞춰 정렬되게 한다("걸린 들여쓰기").
    const lines = u.items.map((m, i) => {
      const nameHtml = i === 0
        ? `<span class="dialog-name" style="color:${nameColor}">${nameText}</span>${whisperTag}`
        : `<span class="dialog-name dialog-name--spacer" aria-hidden="true">${nameText}</span>`;
      return `<div class="group-line" data-id="${m.id}">${dragHandle()}${nameHtml}${renderBubbleItem(m, state)}</div>`;
    }).join('');

    return `<article class="msg-group" data-key="${Utils.escapeHtml(u.key)}">
      ${avatarHtml(u.speaker, u.avatarUrl, avatarColor)}
      <div class="group-main">${lines}</div>
    </article>`;
  }
  ...
  function renderUnit(u, state) {
    ...
    // center (나레이션/시스템)
    const isNarration = m.type === 'desc';
    const emphasis = !!(isNarration && m.emphasis);
    const emphasisColor = (state.display && state.display.emphasisColor) || '#DB7C26';
    const bodyStyle = emphasis ? ` style="color:${emphasisColor}"` : '';
    const centerTypeLabel = emphasis ? '강조 나레이션' : (Utils.TYPE_LABELS[m.type] || m.type);
    // "나레이션" 태그는 클릭해서 강조 나레이션 여부를 직접 켜고 끌 수 있다
    // (원본에 색이 있으면 자동으로 감지하지만, 놓친 경우 여기서 수동으로 바꿀 수 있음)
    const centerTag = isNarration
      ? `<span class="msg-type-tag msg-type-tag--${m.type}" data-action="toggle-emphasis" data-id="${m.id}"
          style="cursor:pointer;" title="클릭하면 강조 나레이션을 ${emphasis ? '해제' : '적용'}해요">${centerTypeLabel}</span>`
      : '';

    return `<article class="msg-row msg-row--center${emphasis ? ' msg-row--center-emphasis' : ''}" data-id="${m.id}">
      ${dragHandle()}
      <div class="center-wrap">
        ${m.speaker ? `<div class="center-speaker">${Utils.escapeHtml(m.speaker)}</div>` : ''}
        <div class="center-body" data-action="edit-message" data-id="${m.id}"${bodyStyle}>${highlighted}</div>
      </div>
      ${centerTag}
      ${delBtn(m.id)}
    </article>`;
  ...
  function renderCharacters(state) {
    ...
    return `<ul class="char-list">` + chars.map(c => {
      if (c.isNarration) {
        // 화자 없는 나레이션(desc) 묶음 — 이름 변경/역할 지정/아바타는 없고
        // 선택해서 다른 캐릭터처럼 일괄삭제만 가능하다.
        return `
      <li class="char-row char-row--narration" data-name="${Utils.escapeHtml(c.name)}">
        ...
        <span class="char-avatar-ph" style="background:${c.color}">✎</span>
        <span class="char-name" title="화자 없는 나레이션 메시지 묶음">${Utils.escapeHtml(c.name)}</span>
        <span class="char-role char-role--fixed">나레이션</span>
        <span></span>
